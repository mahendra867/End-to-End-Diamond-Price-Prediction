...
@app.route('/') # here i have created 1st URL in order to display  html code content for the index.html file in the local host server   
def home_page(): # here i have defined home_page() fucntion which returns the index.html render_template
    return render_template("index.html")

@app.route("/predict",methods=["GET","POST"]) # here i have created 2nd URL which we given URL name as predict in order to display  form.html code content for the form.html file in the local host server , so basically this form.html file contain the html code which helps us to take or fetch or GET the values from the users thats why we mention GET method  and our best model which is in prediciotn pipeline by using the POST method our model predicts those fetched values 
def predict_datapoint():
    if request.method=="GET":  # here by using this GET method to the 2nd url which is /predict iam sending the requesting to the backend code which is form.html so as soons as i click on submit button it sends request to the backend code which is form.html
